...
- 「時価総額予測」タブでは、監視銘柄ごとに現在の時価総額と、販売個数・販売単価の増減率(それぞれ増加・減少どちらも入力可)、本決算までの残り四半期数を入力すると、次の四半期時点・次の本決算時点の見込み時価総額を試算できます。詳しくは下記「時価総額予測の仕組み」を参照してください
...
「時価総額予測」タブは、監視銘柄ごとに次の4つの値を入力すると、次の四半期時点・次の本決算時点の見込み時価総額を試算する機能です。

- 現在の時価総額(億円)
- 販売個数(手数料の量・サービス提供数・ライセンス数など)の増減率(%、マイナス=減少も入力可)
- 販売単価(手数料単価・サービス単価・ライセンス単価など)の増減率(%、マイナス=減少も入力可)
- 本決算までの残り四半期数(既定4)

計算式は `src/lib/market-cap-forecast.ts` の `calculateForecast()` にある通り、増収率 = (1+個数変化率)×(1+単価変化率)-1 を求め、時価総額はこの増収率に比例して動く(バリュエーション倍率は一定)と仮定した単純なモデルです。

> **既知の制約**: これは AI が自然文的に将来を予測しているわけではなく、入力値を単純な数式に当てはめた透明な試算です。現在の時価総額も外部データソースから自動取得しておらず、利用者の手入力が必要です(無料で商用利用可能な時価総額 API がこの開発環境から検証できなかったため)。入力値はこの端末のブラウザ内(localStorage)にのみ保存されます。詳しくは `docs/product-requirements.md` を参照してください。
