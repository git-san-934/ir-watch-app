...
- 「監視銘柄」タブとは別に「自社株買い(全銘柄)」タブがあり、登録銘柄に関係なく**全上場企業**の自己株式取得(自社株買い)関連の開示(取得開始・経過報告など)をまとめて確認できます(`src/lib/tdnet.ts` の `isTreasuryStockDisclosure` — タイトルに「自己株式」+「取得」「買付」等のキーワードを含むものを抽出)
- 各開示情報は × ボタンで個別に非表示(削除)にできます(どちらのタブから消しても、もう一方のタブにも反映されます)
...
- ブラウザが一度取り込んだ開示情報は、× で削除するまで**永続的に**残ります。「監視銘柄」「自社株買い(全銘柄)」はそれぞれ別のアーカイブ(`src/lib/watchlist.ts` の `mergeArchivedDisclosures` / `mergeTreasuryStockArchive`)として保存されますが、非表示(削除)の記録は共通なので、片方のタブで消すともう片方でも消えます
